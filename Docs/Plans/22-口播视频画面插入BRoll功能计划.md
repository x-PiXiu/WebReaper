...
音频轨：  [──────────────口播音频连续播放（全程不断）──────────────]
                ↑ 正常音量          ↑ 降低为背景声(默认30%)    ↑ 恢复正常
```

### 1.2 效果定义（验收基线）

1. **口播画面期间**：口型与音频严格对齐（现状能力，不受影响）
2. **插入片段期间**：画面完全替换为片段内容；口播声音降低为背景声；**不需要口型**（镜头不在人脸上）
...
ffmpeg -i main.mp4 -i broll.mp4 -filter_complex "
  ...
  [0:a]volume='1-DUCK*between(t,START,END)':eval=frame[a]
" -map "[v]" -map "[a]" -c:v libx264 -preset veryfast -c:a aac out.mp4
```
...
| `volume='1-DUCK*between(t,START,END)'` | 窗口内口播降为背景声 | DUCK=0.7 即降到 30% 音量；`eval=frame` 按帧求值 |
...
ffmpeg -i main.mp4 -i b1.mp4 -i b2.mp4 -filter_complex "
  ...
  [0:a]volume='1-0.7*(between(t,S1,E1)+between(t,S2,E2))':eval=frame[a]
" ...
```

音量表达式 `1-0.7*(win1+win2)` 天然处理窗口重叠（重叠时更小，但产品上应校验禁止重叠）。
...
| 方案 | 做法 | 优点 | 缺点 | 结论 |
|---|---|---|---|---|
| **C1. 分句 TTS（选定）** | 音频生产改为按句合成：每句一次 TTS → 记录每句时长 → 拼接成整段音频（与现状等效）+ 时间轴副产品精确已知 | 时间轴**精确到毫秒**；分句天然支持每句音色/语速调优预留；改动集中在管线音频段 | 句间停顿由拼接参数决定（固定静音 300ms 可调），语气连贯性略弱于整段合成；TTS 调用次数=句数（成本/限流考量） | ✅ 主方案 |
| C2. ASR 时间戳对齐 | 整段 TTS 不动；成片音频过支持时间戳的 ASR（whisper 类），句文本对齐出时间轴 | 不动现有 TTS 管线 | 现有 ASR 接口只返回文本无时间戳（`port.SpeechTranscriber` 需扩展）；对齐精度受 ASR 质量/标点影响（±200ms 级） | 备选（ASR 能力升级后可切换） |
| C3. 字符时长估算 | 按每句字符数 × 平均语速估算 | 零成本 | 误差大（±1s 级），切换点会切在字中间 | ❌ 否决 |

### C1 实施要点

1. 管线音频段：`script 按行 split` → 逐句 `AudioSynthesizer.Synthesize`（并发或串行）
   → 每段 `ffprobe` 取精确时长 → `concat` 拼接（句间插入 250~350ms 静音，参数可调）
2. 时间轴数据结构随任务产出持久化：`[{index, text, start_ms, end_ms}]`
3. **兼容策略**：分句合成作为可配置开关（`segmented_tts=true`），默认开启新任务；
   已有的整段音频任务走 C2 兜底或不支持插入（提示重新生成）
4. 停顿时长计入上一句的 end 之后、下一句 start 之前（切换点落在静音处更自然）
...
type InsertSegment struct {
    ...
    DuckVolume    float64 `json:"duck_volume"`    // 窗口内口播音量 0~1（默认 0.3）
...
### 5.4 API 设计（挂现有统一生成体系）

```
POST /api/v1/generation/submit          type=compose（统一提交扩展）
  { source_task_id | source_video_url, segments: [{sentence_index, media_url, duck_volume, fit_mode}] }
GET  /api/v1/generation/tasks/:id       （现有轮询端点，compose 任务同构）
GET  /api/v1/generation/tasks/:id/timeline   （新增：返回该成片的台词时间轴，前端渲染句列表用）
```
...
### 阶段二：分句 TTS 时间轴（管线改造，§4 C1）

- TTS 管线分句开关：分句合成 + 静音拼接 + TimelineLine 持久化
- 时间轴查询端点
- **验证**：同一文案，整段 vs 分句合成产物对比（时长一致性 ±5%）；时间轴抽样人工核对
  （播放器逐句定位）
...
| 2 | 分句 TTS 语气连贯性弱于整段 | 句间静音可调（默认 300ms）；保留整段开关；上线后 A/B 用户反馈 |
| 3 | TTS 分句调用次数增加（限流/成本） | 逐句串行+退避；句数上限校验（如 ≤100 句） |
...
3. **音频**：窗口内口播降至配置音量（默认 30%）且无爆音/咔哒（eval=frame 平滑）；
   窗口外恢复原音量
...
| # | 决策 | 建议 default |
|---|---|---|
| 1 | 片段期间是否混入片段原声（B-roll 自带声音） | 本期不混（口播独占）；`duck_volume` 已参数化，后续加 `mix_broll_audio` 开关 |
