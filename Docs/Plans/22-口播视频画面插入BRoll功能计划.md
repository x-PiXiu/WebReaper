...
### 方案对比（2026-08-29 二次修订：ASR 时间戳方案被实测否决，主方案改为静音检测对齐）

> 修订原因：勘查实际配置——项目当前 ASR 栈是硅基流动 `FunAudioLLM/SenseVoiceSmall`
> （备选小米 MiMo chat 协议），**SenseVoice 模型只返回纯文本、不产生时间戳**，
> `response_format=verbose_json` 是 OpenAI Whisper 协议能力、对当前服务商无效
> （用户实锤质疑，配置勘查确认）。

| 方案 | 做法 | 优点 | 缺点 | 结论 |
|---|---|---|---|---|
| **C3. 静音检测对齐（选定，已实测）** | ffmpeg `silencedetect` 检测 TTS 音频的句间静音边界 → 得到语音段起止时间 → 与台词句**按顺序配对**（只靠数量与顺序，零文本识别） | **完全本地零 API 成本/零服务商依赖**；对 TTS 纯净人声极可靠；边界精度**毫秒级**（实测 2.20475s，优于 whisper ±100ms）；处理快（本地秒级）；双生成入口通吃（TTS/Vidu 端到端的音频都是纯净人声） | 要求音频人声为主——本场景天然满足（音频就是 TTS 产的）；段数≠句数时需按字符比例合并/拆分兜底 | ✅ 主方案 |
| C2. ASR 时间戳对齐 | 带时间戳 ASR（whisper 类 verbose_json） | 通用（任何音频） | **当前 ASR 栈无此能力**（SenseVoice 无时间戳）；需另配 whisper 端点；精度 ±100ms 弱于 C3；有 API 成本 | ⬇️ 降为备选（未来源含 BGM/噪声时启用，需先配 whisper 服务商） |
| C1. 分句 TTS | 每句单独 TTS → 拼接 + 时间轴副产品 | 时间轴毫秒精确 | 每句一次 TTS 调用（成本/限流高风险——用户已否决）；句间停顿固定语气不连贯；需改造管线 | 备选（句级音色调优需求出现时再启用） |
| C4. 字符时长估算 | 字符数 × 平均语速 | 零成本 | 误差 ±1s 级，切换点会切在字中间 | ❌ 否决 |

### C3 实施要点（静音检测对齐，已实测验证）

**实测数据**（2026-08-29，`data/chinese_test.mp3` 真实 TTS 音频 5.69s）：

```
ffmpeg -i audio.mp3 -af silencedetect=noise=-35dB:d=0.15 -f null -
  → silence_start: 2.20475 / silence_end: 2.538583（句间停顿 334ms）
  → 语音段1 = [0, 2.204]  语音段2 = [2.538, 4.989]
```

1. **检测参数**：`noise=-35dB`（TTS 底噪极低，-35dB 足够分辨）、`d=0.15`
   （最短停顿 150ms——TTS 句间停顿通常 200~500ms，词间微停顿 <100ms 被过滤）
2. **配对规则**（语音段数 M 与台词句数 N 的关系）：
   - `M == N`（最常见）：顺序一一配对
   - `M > N`（句中长停顿被切开）：按台词句字符数比例为界合并相邻段
   - `M < N`（相邻句连读无停顿）：按字符数比例拆分段
   - 比例分配的字符数来自台词文本（已知），只切时间不猜内容
3. **解析实现**：解析 silencedetect 的 stderr 行（`silence_start/end` 成对），
   首段前如有语音从 0 起、尾静音截断；产出 `TimelineLine[]` 后与 C2 相同的缓存策略
4. **失败降级**：M 与 N 差异超过 ±30%（音频异常/非 TTS 源）→ 报错引导
   （提示源音频不适合自动定位），未来接 C2 whisper 兜底

### C2 备选实施要点（ASR 时间戳对齐——当前栈不可用，留作未来兜底）

> 前提：ASR 服务商支持 `verbose_json` 时间戳（whisper 类模型）。当前默认配置
> 硅基流动 SenseVoiceSmall **不支持**；启用本路径需在管理后台另配 whisper 端点。

1. **ASR 接口扩展**（若启用）：`port.SpeechTranscriber` 新增 `TranscribeTimestamped`
   （`TranscriptSegment{text, start_ms, end_ms}`）——adapter 层 OpenAI 兼容请求加
   `response_format=verbose_json` 并解析 `segments[]`；不支持的服务商明确报错引导
2. **对齐算法**：文本归一化 → ASR segments 连接整串记录字符区间→时间映射 →
   台词句在整串中顺序定位（编辑距离兜底）→ 句区间映射 [start_ms, end_ms]
3. **适用场景**：源音频含 BGM/噪声（非 TTS 产物）时 C3 静音检测不可靠，
   本路径作为通用兜底
...
### 阶段二：静音检测台词定位（管线零改动，§4 C3）

- silencedetect 解析器：抽音轨（复用 ExtractAudio）→ `ffmpeg -af silencedetect` →
  静音边界解析 → 语音段序列；配对规则（M==N 直配 / M>N 按字符比例合并 / M<N 按比例拆分）
- timeline 定位/读取端点（POST/GET）+ 结果随任务缓存
- **验证**：真实 TTS 成片定位后，播放器按时间轴逐句跳转核对（±200ms）；
...
| 2 | 静音检测参数误切（TTS 语速极快句间停顿 <150ms，或句中长停顿 >150ms） | d 参数可调（0.15s 起步）；段/句数比例配对兜底合并拆分；差异 >±30% 报错引导 force 重跑 |
| 3 | 源音频含 BGM/噪声（非 TTS 产物）静音检测失效 | 定位时检测信噪特征，不可靠时报错；C2 whisper 兜底路径（需另配服务商） |
